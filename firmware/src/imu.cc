...
IMU::IMU(I2cDevice* i2c, char address) : i2c_(i2c), address_(address) {}
...
int IMU::ReadThreeAxisData(
    ...
    char data[6];
    int error = this->i2c_->readBytes(this->address_,
                                baseAddress,
                                6 * sizeof(char),
                                data);
...
int IMU::Initialize() {
    ...
    error = this->i2c_->sendByte(this->address_,
    ...
    // Reset the chip
    error = this->i2c_->sendByte(this->address_, BNO055_SYS_TRIGGER_ADDR, 0x20);
    ...
    do {
        ...
        error = this->i2c_->readByte(this->address_, BNO055_CHIP_ID_ADDR, &response);
    ...
    error = this->i2c_->sendByte(this->address_, BNO055_PAGE_ID_ADDR, 0x00);
    ...
    error = this->i2c_->sendByte(this->address_, BNO055_SYS_TRIGGER_ADDR, 0x00);
...
int IMU::GetTemperature(char* temperature) {
    return this->i2c_->readByte(address_, BNO055_TEMP_ADDR, temperature);
}
